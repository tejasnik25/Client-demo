    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        <Input
          ...
          {...register('capital')}
        />
        <p className="text-xs text-gray-600 mt-1">Allowed for {selectedPlan}: {range.label}</p>
        {errors.capital && (
      ...
      <div>
        ...
        {payable > 0 && (
          <p className="text-xs text-gray-600 mt-1">Approx ₹{inrAmount.toFixed(2)} at ₹{(rate || DEFAULT_USD_TO_INR)} per $1</p>
        )}
        <p className="mt-2 text-xs text-gray-600">Note: INR conversion shown here is indicative to help users.</p>
      </div>
      ...
      {paymentData?.method?.startsWith('USDT') ? (
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm">
          <p className="text-gray-700">You selected {paymentData?.method}. Use the wallet address shown on the next step.</p>
          {process.env.NEXT_PUBLIC_USDT_WALLET_APP_LINK && (
            <a href={process.env.NEXT_PUBLIC_USDT_WALLET_APP_LINK} className="text-[#00d09c] hover:text-[#00b085] font-medium" target="_blank" rel="noreferrer">Open wallet app</a>
          )}
        </div>
      ) : null}
