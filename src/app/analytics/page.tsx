...
function AnalyticsPageContent() {
  ...
  const [mainTab, setMainTab] = useState<'profits' | 'strategies'>('profits');
  const [profitTab, setProfitTab] = useState<'daily' | 'weekly' | 'monthly'>('daily');
    ...
    <div className="space-y-6">
      <div className="bg-[#283046] rounded-lg p-6">
        ...
        <div className="flex space-x-4 mb-6">
          <button
            className={`flex items-center px-4 py-2 rounded-md ${mainTab === 'profits' ? 'bg-[#7367f0] text-white' : 'bg-[#161d31] text-gray-400'}`}
            onClick={() => setMainTab('profits')}
          >
            <FiTrendingUp className="mr-2" /> Profit Analysis
          </button>
          <button
            className={`flex items-center px-4 py-2 rounded-md ${mainTab === 'strategies' ? 'bg-[#7367f0] text-white' : 'bg-[#161d31] text-gray-400'}`}
            ...
            <div className="bg-[#161d31] rounded-lg p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex space-x-2">
                  <button 
                    className={`px-3 py-1 text-xs rounded-md ${profitTab === 'daily' ? 'bg-[#7367f0] text-white' : 'bg-[#283046] text-gray-400'}`}
                    onClick={() => setProfitTab('daily')}
                  >
                    Daily
                  </button>
                  <button 
                    className={`px-3 py-1 text-xs rounded-md ${profitTab === 'weekly' ? 'bg-[#7367f0] text-white' : 'bg-[#283046] text-gray-400'}`}
                    onClick={() => setProfitTab('weekly')}
                  >
                    Weekly
                  </button>
                  <button 
                    className={`px-3 py-1 text-xs rounded-md ${profitTab === 'monthly' ? 'bg-[#7367f0] text-white' : 'bg-[#283046] text-gray-400'}`}
              ...
              {renderBarChart(
                profitTab === 'daily'
                  ...
                  : profitTab === 'weekly'
                  ? analyticsData.profits.weekly
                  : analyticsData.profits.monthly,
                profitTab === 'daily'
...
export default function AnalyticsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-white">Loading analytics...</div>}>
      <AnalyticsPageInner />
    </Suspense>
  );
}
