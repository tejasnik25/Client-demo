...
export const loginUser = async (
  ...
  try {
    ...
    if (!user) {
      console.error('User not found in JSON store:', email);
...
export const updateUserAdmin = async (
  ...
  try {
    ...
    if (typeof updates.password !== 'undefined') { fields.push('password = ?'); values.push(updates.password); }
    ...
    console.error('MySQL updateUserAdmin failed, falling back to JSON:', error);
    try {
      const db: any = readDatabase();
      const users: any[] = Array.isArray(db.users) ? db.users : [];
      ...
      const updatedUser = {
        ...
        enabled: updates.enabled !== undefined ? updates.enabled : currentUser.enabled,
        phone: updates.phone !== undefined ? updates.phone : currentUser.phone,
        password_updated_at: updates.password_updated_at !== undefined ? (updates.password_updated_at instanceof Date ? updates.password_updated_at.toISOString() : updates.password_updated_at) : currentUser.password_updated_at,
        email_updated_at: updates.email_updated_at !== undefined ? (updates.email_updated_at instanceof Date ? updates.email_updated_at.toISOString() : updates.email_updated_at) : currentUser.email_updated_at,
        updated_at: new Date().toISOString()
      };
      ...
      console.error('JSON fallback updateUserAdmin failed:', jsonError);
...
export const getRunningStrategyById = async (id: string): Promise<{
  id: string;
  userId: string;
  strategyId: string;
  status: string;
  ...
  try {
    const [rows] = await pool.execute(
      `SELECT rs.id, rs.user_id, rs.strategy_id, rs.status, rs.admin_status, rs.plan, rs.capital,
              COALESCE(rsm.platform, wt.platform) AS platform,
              COALESCE(rsm.mt_account_id, wt.mt_account_id) AS mtAccountId,
              COALESCE(rsm.mt_account_password, wt.mt_account_password) AS mtAccountPassword,
              COALESCE(rsm.mt_account_server, wt.mt_account_server) AS mtAccountServer
    ...
    if (arr.length > 0) {
      return {
        ...
        strategyId: arr[0].strategy_id,
        status: arr[0].status,
        adminStatus: arr[0].admin_status,
        plan: arr[0].plan,
        capital: Number(arr[0].capital),
        platform: arr[0].platform || null,
        mtAccountId: arr[0].mtAccountId || null,
        mtAccountPassword: arr[0].mtAccountPassword || null,
        mtAccountServer: arr[0].mtAccountServer || null,
      };
    }
    return null;
  } catch (error) {
    try {
      const db: any = readDatabase();
      const runs: any[] = Array.isArray(db.running_strategies) ? db.running_strategies : [];
      const found = runs.find((r: any) => r.id === id);
      if (found) {
        // For JSON fallback, we might not have the joined data easily, 
        // but let's return what we can or mock it if needed for now.
        // Assuming JSON structure might optionally have these if saved there.
        return {
          id: found.id,
          userId: found.user_id,
          strategyId: found.strategy_id,
          status: found.status,
          adminStatus: found.admin_status,
          plan: found.plan,
          capital: Number(found.capital),
          // Fallback might be missing these details if not stored in running_strategies.json
          platform: found.platform || null,
          mtAccountId: found.mt_account_id || null,
          mtAccountPassword: found.mt_account_password || null,
          mtAccountServer: found.mt_account_server || null,
        };
      }
      return null;
    } catch (e) {
...
export const createRunningStrategy = async (
  ...
  try {
    ...
    try {
      ...
      writeDatabase({ 
        ...db, 
        running_strategies: [...runs, newRun],
        running_strategy_modifications: [...mods, newMod]
      });
      return { success: true, id };
    } catch (e) {
      return { success: false };
...
export const createRunningStrategyModification = async (
  ...
  try {
    ...
    try {
      const db: any = readDatabase();
      const mods: any[] = Array.isArray(db.running_strategy_modifications) ? db.running_strategy_modifications : [];
      const next = [...mods, { ...payload, new_update_json: payload.new_update_json, created_at: new Date().toISOString() }];
      writeDatabase({ ...db, running_strategy_modifications: next });
      return { success: true };
    } catch (e) {
      return { success: false };
    }
  ...
  try {
    const [rows] = await pool.execute(
      `SELECT m.* FROM running_strategy_modifications m
       JOIN running_strategies rs ON rs.id = m.running_strategy_id
       WHERE rs.admin_status IN ('in-process','wrong-account-password','wrong-account-id','wrong-account-server-name')
    ...
    try {
      ...
      return mods
        .filter(m => { const rs = runMap[m.running_strategy_id]; const s = (rs?.admin_status || rs?.adminStatus || '').toLowerCase(); return s && s !== 'running' && s !== 'disconnected'; })
        .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    } catch (e) {
      return [];
    }
...
export const getPendingModificationsForStrategy = async (runningStrategyId: string): Promise<RunningStrategyModificationRow[]> => {
  try {
    const [rows] = await pool.execute(
      `SELECT * FROM running_strategy_modifications 
       WHERE running_strategy_id = ? AND status = 'in-process'
       ORDER BY created_at ASC`,
      [runningStrategyId]
    );
    return rows as RunningStrategyModificationRow[];
  } catch (error) {
    try {
      const db: any = readDatabase();
      const mods: any[] = Array.isArray(db.running_strategy_modifications) ? db.running_strategy_modifications : [];
      return mods
        .filter((m: any) => m.running_strategy_id === runningStrategyId && m.status === 'in-process')
...
export const getRunningStrategyModificationById = async (id: string): Promise<RunningStrategyModificationRow | null> => {
  try {
...
export const getAds = async (): Promise<Ad[]> => {
  try {
    ...
    try {
      const db: any = readDatabase();
      return Array.isArray(db.ads) ? db.ads : [];
    } catch (e) {
      return [];
    }
  }
...
export const getAdById = async (id: string): Promise<Ad | null> => {
  try {
    ...
    try {
      const db: any = readDatabase();
      const ads: Ad[] = Array.isArray(db.ads) ? db.ads : [];
...
export const createAd = async (adData: Omit<Ad, 'id' | 'createdAt' | 'updatedAt'>): Promise<Ad> => {
  ...
  try {
    await pool.execute(
      'INSERT INTO ads (id, title, content, image_url, link_url, position, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, ad.title, ad.content, ad.imageUrl || null, ad.linkUrl || null, ad.position, ad.isActive, now, now]
    );
  } catch (error) {
    try {
      const db: any = readDatabase();
...
export const updateAd = async (id: string, adData: Partial<Omit<Ad, 'id' | 'createdAt'>>): Promise<Ad | null> => {
  const now = new Date().toISOString();
  ...
  try {
    ...
    if (adData.title !== undefined) {
      ...
      values.push(adData.title);
    }
    if (adData.content !== undefined) {
      updateFields.push('content = ?');
      values.push(adData.content);
    }
    if (adData.imageUrl !== undefined) {
      updateFields.push('image_url = ?');
      values.push(adData.imageUrl || null);
    }
    if (adData.linkUrl !== undefined) {
      updateFields.push('link_url = ?');
      values.push(adData.linkUrl || null);
    }
    if (adData.position !== undefined) {
      updateFields.push('position = ?');
      values.push(adData.position);
    }
    if (adData.isActive !== undefined) {
      updateFields.push('is_active = ?');
      values.push(adData.isActive);
    }
    ...
    try {
      ...
      ads[index] = { ...ads[index], ...adData, updatedAt: now };
      writeDatabase(db);
      return ads[index];
    } catch (e) {
      return null;
